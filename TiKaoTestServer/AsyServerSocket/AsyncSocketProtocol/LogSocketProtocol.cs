...
namespace TiKaoTestOne
{
    public class LogOutputSocketProtocol : BaseSocketProtocol
    {
            ...
            : base(asyncSocketServer, asyncSocketUserToken)
        {
            ...
            lock (SocketTiKao.AsyncSocketSvr.LogOutputSocketProtocolMgr)
            {
                SocketTiKao.AsyncSocketSvr.LogOutputSocketProtocolMgr.Add(this);
            }
        ...
        public override void Close()
        {
            lock (SocketTiKao.AsyncSocketSvr.LogOutputSocketProtocolMgr)
            {
                 SocketTiKao.AsyncSocketSvr.LogOutputSocketProtocolMgr.Remove(this);
            }
        }
    ...
    class LogSocketAppender : log4net.Appender.AppenderSkeleton
    {
        ...
        protected override void Append(LoggingEvent loggingEvent)
        {
            ...
            lock (SocketTiKao.AsyncSocketSvr.LogOutputSocketProtocolMgr)
            {
                for (int i = 0; i < SocketTiKao.AsyncSocketSvr.LogOutputSocketProtocolMgr.Count(); i++)
                {
                    SocketTiKao.AsyncSocketSvr.LogOutputSocketProtocolMgr.ElementAt(i).LogFixedBuffer.WriteBuffer(tmpBuffer);
                    SocketTiKao.AsyncSocketSvr.LogOutputSocketProtocolMgr.ElementAt(i).InitiativeSend();
    ...
    //显示操作消息
    class LogListAppender : log4net.Appender.AppenderSkeleton
    {

        public LogListAppender()
        {
            Name = "LogListAppender";
        }

        protected override void Append(LoggingEvent loggingEvent)
        {
            String strLoggingMessage = RenderLoggingEvent(loggingEvent);
            SocketTiKao.ShowMessage(strLoggingMessage);
        }
    }
